...
from lib.base_types import CancellationToken, EventHandler, ServerStreamEvent, StreamEventType
...
class EventSubscription:
    ...
    def __init__(
        ...
        self.queue: asyncio.Queue[ServerStreamEvent] = asyncio.Queue(maxsize=queue_size)
        self.active = True
...
class EventManager:
    ...
    def __init__(self, max_event_history: int = 10000):
        ...
        self._event_history: list[ServerStreamEvent] = []
        self._max_history = max_event_history
        self._subscription_counter = 0
        self._lock = asyncio.Lock()
        self._shutdown = False

    async def publish(self, event: ServerStreamEvent) -> None:
        """
        ...
        async with self._lock:
            ...
            for subscription in self._subscriptions.values():
                if event.type in subscription.event_types and subscription.active:
                    try:
    ...
    async def event_stream(
    ...
    ) -> AsyncIterator[AsyncIterator[ServerStreamEvent]]:
        """
        ...
        """
        queue: asyncio.Queue[ServerStreamEvent] = asyncio.Queue()
        ...
        async def event_iterator() -> AsyncIterator[ServerStreamEvent]:
            try:
                while True:
